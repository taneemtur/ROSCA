...
const RoscaCard = (props:any) => {
    ...
    useEffect(()=>{
        ...
        const length = parsed && Object.keys(parsed).length
         if(length){
            for(let i=0;i<length;i++){
                ...
                if(i===0 && participantObject){
                    setParticipantsArray([participantObject])
                }else setParticipantsArray([...participantsArray,participantObject])
            }
         }
    },[participant])
    ...
    return (
        <div className="">
            ...
            {participantsArray&& participantsArray.map((e,i)=>{
                return(
                    <p>{`Participant ${i+1} : `+ e.address&&e.address}</p>
                )
            })}
        </div>
    )
