    ...
    <div className="mt-16">
      <div className="flex flex-col">
        ...
        <div className='flex justify-between'>
          <p>A list of all the participants.</p>
          {myAddress && moderators.includes(myAddress) && <button onClick={handleModalOpen} className='h-12 bg-blue-600 hover:bg-blue-800 text-white font-semibold py-2 px-4 border border-blue-500 hover:border-transparent rounded-md'>Add Trusted Address</button>}
        </div>
      </div>
      <div className="bg-white border border-[#8F8F8F] mt-4 width-full h-full rounded ">
        ...
        {admins.map((e:any,i:number)=>{
          return <div className={`flex h-20 border-[#C0C0C0] ${i+1!=admins.length&&'border-b-4'} border-dashed width-full justify-between pt-4 pb-4`}>
                    <div className="flex pl-6">
                      <RiAdminFill size={'40px'}/>
                      <p className='pl-4 pt-2 text-xl'>{e}</p>
                    </div>
                    <div className="flex pr-6 items-center">
                      <button onClick={()=>{handleDeleteTrusted(e)}}><MdDelete size={'36px'}/></button>
                    </div>
                  </div>
          })}
      </div>
